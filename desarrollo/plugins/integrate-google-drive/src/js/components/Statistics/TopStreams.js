...
export default function TopStreams({data, startDate, endDate}) {

    const {streams, streamUsers} = data;

    const [showData, setShowData] = useState(localStorage.getItem('igd_show_streams_data') ? localStorage.getItem('igd_show_streams_data') == 'true' : true);
    const [showUsers, setShowUsers] = useState(localStorage.getItem('igd_show_streams_users') ? localStorage.getItem('igd_show_streams_users') == 'true' : true);
    
    return (
        <div className="statistics-boxes top-stream">

            {/* Top Streams */}
            <div className="statistics-box">

                <div className="box-title">
                    <span>{wp.i18n.__('Top Streams', 'integrate-google-drive')}</span>

                    <i className={`dashicons dashicons-download`}
                       onClick={() => exportStatistics(startDate, endDate, 'streams')}
                    ></i>

                    <i className={`dashicons dashicons-arrow-${showData ? 'up' : 'down'}-alt2`}
                       onClick={() => {
                           localStorage.setItem('igd_show_streams_data', !showData);
            ...
            <div className="statistics-box">
                <div className="box-title">
                    <span>{wp.i18n.__('Top Users with most Streams', 'integrate-google-drive')}</span>

                    <i className={`dashicons dashicons-download`}
                       onClick={() => exportStatistics(startDate, endDate, 'stream_users')}
                    ></i>

                    <i className={`dashicons dashicons-arrow-${showUsers ? 'up' : 'down'}-alt2`}
                       onClick={() => {
                           localStorage.setItem('igd_show_streams_users', !showUsers);
