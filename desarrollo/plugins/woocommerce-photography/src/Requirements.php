...
use Themesquad\WC_Photography\Utilities\Plugin_Utils;
	...
	protected static function check_requirements() {
		if ( ! self::is_php_compatible() ) {
			self::$errors[] = sprintf(
				...
				_x( '<strong>Photography for WooCommerce</strong> requires PHP %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-photography' ),
			...
			self::$errors[] = sprintf(
				...
				_x( '<strong>Photography for WooCommerce</strong> requires WordPress %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-photography' ),
				self::MINIMUM_WP_VERSION,
				get_bloginfo( 'version' )
			);
		} elseif ( ! Plugin_Utils::is_woocommerce_active() ) {
			self::$errors[] = _x( '<strong>Photography for WooCommerce</strong> requires WooCommerce to be activated to work.', 'admin notice', 'woocommerce-photography' );
		} elseif ( ! self::is_wc_compatible() ) {
			self::$errors[] = sprintf(
				/* translators: 1: Minimum WooCommerce version 2: Current WooCommerce version */
				_x( '<strong>Photography for WooCommerce</strong> requires WooCommerce %1$s or higher. You are using version %2$s', 'admin notice', 'woocommerce-photography' ),
