...
class WC_Deposits_Order_Manager {
	...
	public function maybe_bypass_manage_stock( $manage_stock, $product ) {
		...
		// Only run this logic on the "pay for order" page, where both 'pay_for_order' and 'key' are present in the URL
		// and 'pay_for_order' is a truthy value (e.g., 'yes', 'true', 1, true). This ensures we only bypass stock management
		// in the correct context, and not just when the parameter is present with any value.
		if ( ! ( isset( $_GET['pay_for_order'], $_GET['key'] ) && wc_string_to_bool( wp_unslash( $_GET['pay_for_order'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			return $manage_stock;
		}
	...
	public function maybe_bypass_stock_status( $in_stock, $product ) {
		// Only run this logic on the "pay for order" page, where both 'pay_for_order' and 'key' are present in the URL
		// and 'pay_for_order' is a truthy value (e.g., 'yes', 'true', 1, true). This ensures we only bypass stock status
		// in the correct context, and not just when the parameter is present with any value.
		if ( ! ( isset( $_GET['pay_for_order'], $_GET['key'] ) && wc_string_to_bool( wp_unslash( $_GET['pay_for_order'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended,WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			return $in_stock;
		}
