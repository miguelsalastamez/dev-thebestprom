...
class WC_Deposits {
	...
	public function display_deposits_activation_notice() {
		$notice_prefix = is_multisite() ? get_current_blog_id() . '_' : '';
		...
		if (
			...
			WC_Admin_Notices::remove_notice( "{$notice_prefix}woocommerce_deposits_activation" );
			return;
		}

		if ( WC_Admin_Notices::user_has_dismissed_notice( "{$notice_prefix}woocommerce_deposits_activation" ) ) {
			return;
		}

		WC_Admin_Notices::add_custom_notice( "{$notice_prefix}woocommerce_deposits_activation", $this->get_deposits_activation_notice() );
	}
