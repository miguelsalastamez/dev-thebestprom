	...
	function pafe_ajax_form_builder() {
		...
		global $wpdb;
			...
			if ( !empty($_POST['post_id']) && !empty($_POST['form_id']) && !empty($_POST['fields']) ) {
                ...
                $attachment_signature_to_email = [];
                $attachment_signature_to_email2 = [];
				...
				foreach ($fields as $key_field=>$field) {
					...
					if (isset($fields[$key_field]['attach-files'])) {
                        ...
                        if($fields[$key_field]['type'] == 'signature'){
                            $attachment_images_to = json_decode($fields[$key_field]['attach-files']);
                            $sing_image = !empty($fields[$key_field]['value']) ? $fields[$key_field]['value'] : false;
                            if($sing_image){
                                $image_sign_name = basename( $sing_image );
                                if(in_array('email', $attachment_images_to)){
                                    $attachment_signature_to_email[] = wp_get_upload_dir()['path'] . '/'. $image_sign_name;
                                }
                                if(in_array('email2', $attachment_images_to)){
                                    $attachment_signature_to_email2[] = wp_get_upload_dir()['path'] . '/'. $image_sign_name;
                                }
                            }
                        }
						if ($fields[$key_field]['attach-files'] == 1) {
...
', $field['value']) ),
								...
								if($type == 'image'){
									...
									if(wp_http_validate_url($pdf_image_url)){
                                        ...
                                        if(!empty($item['pdf_remove_signature'])){
                                            ...
                                            pafe_delete_acttachment($signature_url);
					...
					if (in_array("email", $form['settings']['submit_actions']) && $failed == false) {
						...
						$status = wp_mail( $to, $subject, $message . $meta_content, $headers, array_merge($attachment, $attachment_images_to_email, $attachment_signature_to_email) );
					...
					if (in_array("email2", $form['settings']['submit_actions']) && $failed == false) {
						...
						$status = wp_mail( $to, $subject, $message . $meta_content_2, $headers_email, array_merge($attachment, $attachment_images_to_email2, $attachment_signature_to_email2) );
