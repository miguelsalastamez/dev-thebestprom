...
if ( $style == 'style-1' || $style == 'style-2' || $style == 'style-3' ) {
	$events_html .= '
    <div id = "ect-accordion-event-' . esc_attr( $event_id ) . '"' . $cat_colors_attr . ' class="ect-accordion-event ' . $style . ' ' . $event_type . '" itemscope itemtype="http://schema.org/Event">
	...
	if ( tribe_get_cost( $event_id, true ) ) {
		$events_html .= '<div class="ect-accordion-cost">' . $ev_cost . '</div>';
			...
			$events_html .= '<a href="' . esc_url( tribe_get_event_link( $event_id ) ) . '" class="ect-events-read-more" rel="bookmark" >' . $events_more_info_text . '</a>
			</div>';
		$events_html     .= '<div class="ect-accordion-image">
                    <img src="' . $ev_post_img . '" title="' . get_the_title( $event_id ) . '" alt="' . get_the_title( $event_id ) . '"></div>';
	if ( $style != 'style-1' ) {
