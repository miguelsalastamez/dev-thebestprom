...
class GFPolls extends GFAddOn {
	...
	public function scripts() {
		$scripts = array(
			array(
				...
				'src'     => $this->get_base_url() . "/js/gpoll_form_editor{$this->_asset_min}.js",
			...
			array(
				...
				'src'     => $this->get_base_url() . "/js/gpoll_form_settings{$this->_asset_min}.js",
				'version' => $this->_version,
				'deps'    => array( 'jquery' ),
				'enqueue' => array(
					array(
			...
			array(
				'handle'   => 'gpoll_js',
				'src'      => $this->get_base_url() . "/js/gpoll{$this->_asset_min}.js",
				'version'  => $this->_version,
				'deps'     => array( 'jquery' ),
		...
		if ( ! empty( $merge_tags ) ) {
			$scripts[] = array(
				...
				'src'     => $this->get_base_url() . "/js/gpoll_merge_tags{$this->_asset_min}.js",
				'version' => $this->_version,
				'deps'    => array( 'jquery' ),
				'enqueue' => array(
	...
	public function styles() {
		...
		if ( ! $this->supports_theme_enqueuing() ) {
			$styles[] = array(
				...
				'version' => $version,
				'enqueue' => array(
					array( 'admin_page'  => array( 'form_editor', 'block_editor', 'entry_view', 'entry_detail' ) ),
					array( 'field_types' => array( 'poll' ) ),
				),
			);
			$styles[] = array(
				'handle'  => 'gform-polls-theme-framework',
	...
	public function theme_layer_styles( $form, $ajax, $settings, $block_settings = array() ) {
		return $this->supports_theme_enqueuing() ? $this->get_theme_layer_styles( $form, 'poll' ) : array();
	}
		...
		if ( $field instanceof GF_Field && $field->type == 'poll' ) {
			if ( $field->is_entry_detail() ) {
				$results                 = $this->gpoll_get_results( $form['id'], $field->id, 'green', true, true, $entry );
 				$new_value               = sprintf( '<div class="gpoll_entry gravity-theme">%s</div>', rgar( $results, 'summary' ) );
	...
	public function upgrade( $previous_version ) {
		$previous_is_pre_addon_framework = ! empty( $previous_version ) && version_compare( $previous_version, '1.5.4', '<' );
	...
	public function poll_field_settings( $position, $form_id ) {
		...
		if ( $position == 25 ) {
			...
			<li class="randomize_choices_setting field_setting">
				...
				<label for="field_randomize_choices" class="inline"><?php esc_html_e( 'Randomize order of choices', 'gravityformspolls' ); ?><?php gform_tooltip( 'form_field_randomize_choices' ) ?></label>
	...
	public function gpoll_get_results( $formid, $display_field = '0' /* zero = all fields */, $style = 'green', $show_percentages = true, $show_counts = true, $lead = array() ) {
		...
		$bar_html = "<div class='gpoll_wrapper {$style}'>%s<div class='gpoll_bar'>";
		$bar_html .= "<span class='gpoll_bar_juice' data-origwidth='%s' style='width: %s%%'>%s</span>%s</div></div></div><!-- .gpoll_choice_wrapper -->";

		// the percentage label and count will be displayed inside the bar
		if ( class_exists( 'GFCommon' ) && GFCommon::is_entry_detail() ) {
			$ratio_box = "<div class='gpoll_ratio_box' aria-hidden='true' value='%s'><div class='gpoll_ratio_label'>%s</div></div>";
		} else {
			$ratio_box = "<div class='gpoll_ratio_box' aria-hidden='true' style='inset-inline-start: %s'><div class='gpoll_ratio_label'>%s</div></div>";
		}
		$bar_count           = "<span class='gpoll_bar_count' aria-hidden='true'>%s</span>";
		$screen_reader_count = "<span class='screen-reader-text'>%s</span>";
		...
		foreach ( $gpoll_data['fields'] as $field ) {
			...
			$gpoll_output['summary'] .= "<div class='gpoll_field' role='region' aria-labelledby='poll_label_{$fieldid}'>";
			$gpoll_output['summary'] .= "<div class='gpoll_field_label_container'>";

			$gpoll_output['summary'] .= "<h3 id='poll_label_{$fieldid}' class='gpoll_field_label'>";
			$gpoll_output['summary'] .= $field['field_label'] . '</h3></div>';
			...
			foreach ( $field['inputs'] as $input ) {
				...
				$form_theme = self::get_form_theme( $formid );
				if ( $form_theme === 'orbital' ) {
					$offset_rem = ( $ratio < 10 ) ? '1.25rem' : '1.75rem';
    				$inset_style = sprintf( "calc(%s%% - %s);", $ratio, $offset_rem );
				} else {
					$inset_style = null;
				}
				...
				if ( in_array( rgar( $input, 'value' ), $selected_values ) ) {
					...
					$selected_element   = '<span class="screen-reader-text">' . esc_html__('Selected Choice', 'gravityformspolls') . '</span>';
				}

				if ( $show_percentages === true  ) {
					$percentage_label = sprintf( $ratio_box, $inset_style, $ratio . '%' );
				} else {
					$percentage_label = $form_theme === 'gravity-theme' ? sprintf( $ratio_box, "calc(0%% - 1.75rem);", '' ) : '';
				}

				//build the bar and add it to the summary
				$gpoll_output['summary'] .= sprintf( "<div class='gpoll_choice_wrapper'><div class='gfield_description gpoll_choice_label%s'>%s%s</div>", $selected_class, rgar( $input, 'label' ), $selected_element );
				$count                   = $show_counts === true ? sprintf( $bar_count, rgar( $input, 'total_entries' ) ) : '';
				$screen_reader_label     = $count && $percentage_label
					? sprintf( $screen_reader_count, sprintf( esc_html__( '%s: %s%% of votes, %s votes', 'gravityformspolls' ), rgar( $input, 'label' ), $ratio, rgar( $input, 'total_entries' ) ) )
                	: ( $count ? sprintf( $screen_reader_count, sprintf( esc_html__( '%s: %s votes', 'gravityformspolls' ), rgar( $input, 'label' ), rgar( $input, 'total_entries' ) ) )
					: ( $percentage_label ? sprintf( $screen_reader_count, sprintf( esc_html__( '%s: %s%% of votes', 'gravityformspolls' ), rgar( $input, 'label' ), $ratio ) )
					: '' ) );

				$input_html              = sprintf( $bar_html, $percentage_label, $ratio, $ratio, $count, $screen_reader_label );
	...
	/**
	 * Retrieves the theme slug.
	 *
	 * Determines the theme to used for the form based on the submitted form ID and request type.
	 *
	 * @since 4.3.2
	 *
	 * @param int $formid The ID of the form.
	 * @return string The theme slug for the form.
	 */
	private static function get_form_theme( $formid ) {
		$form             = GFFormsModel::get_form_meta( $formid );
		$submitted_formid = rgpost( 'action' ) === 'gpoll_ajax' ?  intval( rgpost( 'formId') ) : intval( rgpost( 'gform_submit') );
		$form_theme       = $submitted_formid === $formid ? rgpost('gform_theme') : GFFormDisplay::get_form_theme_slug( $form );
	...
	public function build_poll_ui( $form_id, $field_id, $style, $mode, $percentages, $counts, $title, $description, $confirmation, $show_results_link, $ajax, $cookie, $display_results, $field_values, $disable_scripts, $tabindex, $return = true ) {
		...
		if ( $mode == 'results' ) {
			...
			// Call to get_form() to trigger enqueueing of scripts and styles.
			GFForms::get_form( $form_id );

			$results           = $this->gpoll_get_results( $form_id, $field_id, $style, $percentages, $counts );
            $default_theme     = method_exists( 'GFForms', 'get_default_theme' ) ? GFForms::get_default_theme() : 'gravity-theme';
            $theme_css_classes = $default_theme === 'gravity-theme' ? 'gravity-theme gform-theme--no-framework' : 'gform-theme gform-theme--foundation gform-theme--framework gform-theme--orbital';
			$output            = "<div class='gform_wrapper {$theme_css_classes}'>" . $results['summary'] . "</div>";
			...
			if ( $disable_scripts === false ) {
				GFForms::print_form_scripts( $form, $ajax );
			}
			...
			$output = GFForms::get_form( $form_id, $title, $description, false, $field_value_array, $ajax, $tabindex );
		}
