...
class Table extends \WP_List_Table {
	...
	public function __construct() {
		...
		if ( $screen instanceof \WP_Screen ) {
			$screen->add_option(
				'per_page',
				[
					'label'  => __( 'Number of payouts per page:', 'tribe-events-community-tickets' ),
	...
	public function get_columns() {
		$columns = [
			'tickets'    => __( 'Tickets', 'tribe-events-community-tickets' ),
			'receiver'   => __( 'Receiver', 'tribe-events-community-tickets' ),
			'order_date' => __( 'Order Date', 'tribe-events-community-tickets' ),
			'date_paid'  => __( 'Date Paid', 'tribe-events-community-tickets' ),
			'amount'     => __( 'Payout', 'tribe-events-community-tickets' ),
			'status'     => __( 'Status', 'tribe-events-community-tickets' ),
		];
	...
	public function column_tickets( $item ) {
		...
		$order_text = sprintf( __( 'Order #%s', 'tribe-events-community-tickets' ), $order_id );
	...
	public function column_receiver( $item ) {
		...
		return __( 'Unknown', 'tribe-events-community-tickets' );
	}
	...
	public function column_status( $item ) {
		...
		if ( null === $status_label ) {
			$status_label = __( 'Unknown', 'tribe-events-community-tickets' );
		}
		...
		if ( ! empty( $error_message ) ) {
			$status_details .= sprintf(
				'
					...
					<em>%1$s: %2$s</em>
				',
				esc_html__( 'Note', 'tribe-events-community-tickets' ),
		...
		if ( ! empty( $transaction_id ) ) {
			$status_details .= sprintf(
				'
					...
					<em>%1$s: %2$s</em>
				',
				esc_html__( 'Transaction ID', 'tribe-events-community-tickets' ),
