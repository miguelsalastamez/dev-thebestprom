...
export default function TopPreviews({data, startDate, endDate}) {
    const [showData, setShowData] = useState(localStorage.getItem('igd_show_previews_data') ? localStorage.getItem('igd_show_previews_data') === 'true' : true);
    const [showUsers, setShowUsers] = useState(localStorage.getItem('igd_show_previews_users') ? localStorage.getItem('igd_show_previews_users') === 'true' : true);

    const {previews, previewUsers} = data;

    return (
        <div className="statistics-boxes top-preview">

            {/* Top Downloads */}
            <div className="statistics-box">

                <div className="box-title">
                    <span>{wp.i18n.__('Top Previews', 'integrate-google-drive')}</span>

                    <i className={`dashicons dashicons-download`}
                       onClick={() => exportStatistics(startDate, endDate, 'previews')}
                    ></i>

                    <i className={`dashicons dashicons-arrow-${showData ? 'up' : 'down'}-alt2`}
                       onClick={() => {
                           localStorage.setItem('igd_show_previews_data', !showData);
                        ...
                        <table className="widefat top-items striped">
                            ...
                            <tbody>
                            {
                                previews.map((item, index) => (
                                    <tr key={index}>
                                        <td>
                                            <div className="sl"><span className="sl-no">{index + 1}.</span> <img
                                                width={24}
                                                src={getTypeIcon(item.file_type)}/>
                                            </div>
                                        </td>
                                        <td><a
            ...
            <div className="statistics-box">
                ...
                <div className="box-title">
                    <span>{wp.i18n.__('Top Users with most Previews', 'integrate-google-drive')}</span>

                    <i className={`dashicons dashicons-download`}
                       onClick={() => exportStatistics(startDate, endDate, 'previews_users')}
                    ></i>

                    <i className={`dashicons dashicons-arrow-${showUsers ? 'up' : 'down'}-alt2`}
                       onClick={() => {
                           localStorage.setItem('igd_show_previews_users', !showUsers);
