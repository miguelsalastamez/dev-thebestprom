 ...
 *
 */
if ( ! class_exists( 'ECTCSF_Shortcoder' ) ) {
  class ECTCSF_Shortcoder extends ECTCSF_Abstract{
    ...
    public $args         = array(
      ...
      'class'            => '',
      'gutenberg'        => array(
        'title'          => 'ECTCSF Shortcodes',
        'description'    => 'ECTCSF Shortcode Block',
        'icon'           => 'screenoptions',
        'category'       => 'widgets',
        'keywords'       => array( 'shortcode', 'ectcsf', 'insert' ),
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_ectcsf-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );
      ...
      if ( ! empty( $this->args['show_in_editor'] ) ) {
        ...
        if ( ECTCSF::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'ECTCSF', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'ECTCSF_Field_icon', 'add_footer_modal_icon' ) );
    ...
    public function add_footer_modal_shortcode() {
      ...
      if( ! wp_script_is( 'ectcsf' ) ) {
        return;
      }

      $class        = ( $this->args['class'] ) ? ' '. esc_attr( $this->args['class'] ) : '';
      $has_select   = ( count( $this->pre_tabs ) > 1 ) ? true : false;
      $single_usage = ( ! $has_select ) ? ' ectcsf-shortcode-single' : '';
      $hide_header  = ( ! $has_select ) ? ' hidden' : '';

    ?>
      <div id="ectcsf-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui ectcsf-modal ectcsf-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'ectcsf_shortcode_nonce' ) ); ?>">
          ...
          <div class="ectcsf-modal-table-cell">
            ...
            <div class="ectcsf-modal-inner">
              <div class="ectcsf-modal-title">
                ...
                <div class="ectcsf-modal-close"></div>
              </div>
              <?php

                echo '<div class="ectcsf-modal-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="ectcsf-modal-insert-wrapper hidden"><a href="#" class="button button-primary ectcsf-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'ectcsf_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              ECTCSF::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'ectcsf' );
            $inner_shortcode = ( ! empty( $section['group_shortcode'] ) ) ? $section['group_shortcode'] : $shortcode;

            echo '<div class="ectcsf--repeatable">';

              echo '<div class="ectcsf--repeat-shortcode">';

                echo '<div class="ectcsf-repeat-remove fas fa-times"></div>';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  ECTCSF::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="ectcsf--repeat-button-block"><a class="button ectcsf--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. $button_title .'</a></div>';
        ...
        echo '<div class="ectcsf-field ectcsf-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'ectcsf' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'ECTCSF_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( ectcsf_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'ECTCSF_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'ectcsf-gutenberg-block', ECTCSF::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( ECTCSF::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary ectcsf-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. $value['button_title'] .'</a>';
      }
