...
>
	<div class="jet-engine-edit-page__fields">
		...
		<div class="cx-vui-panel">
			...
			></cx-vui-select>
			<cx-vui-input
				label="<?php _e( 'Custom Query ID', 'jet-engine' ); ?>"
			...
			<cx-vui-switcher
				...
				:class="{
					'cx-vui-component--disabled': !isCacheable()
				}"
			></cx-vui-switcher>
			<cx-vui-input
				v-if="isCacheable()"
				label="<?php _e( 'Cache Expires', 'jet-engine' ); ?>"
				description="<?php _e( 'Object cache expiration time in seconds. Default 0 (no expiration). Please note: this option makes sense only when external Object cache used (Redis, Memcached, etc.). In other case cache is actual during single request.', 'jet-engine' ); ?>"
				:wrapper-css="[ 'equalwidth' ]"
				size="fullwidth"
			...
			></cx-vui-input>
			<cx-vui-component-wrapper
				v-if="isAvoidDuplicates()"
				label="<?php _e( 'Warning!', 'jet-engine' ); ?>"
				description="<?php _e( 'Caching cannot be enabled as you have \'Avoid Duplicates\' setting enabled.', 'jet-engine' ); ?>"
			></cx-vui-component-wrapper>
			<cx-vui-switcher
	...
	<div class="jet-engine-edit-page__actions">
		<div class="jet-engine-edit-page__actions-panel">
			...
			?></div>
			<div class="jet-engine-edit-page__actions-buttons" v-if="!generalSettings.show_preview">
			...
			<div class="jet-engine-query-preview" v-if="generalSettings.show_preview">
				...
				<div class="jet-engine-query-preview__settings">
					<div class="cx-vui-component cx-vui-component--preview-control">
						<div class="jet-engine-edit-page__actions-buttons">
							...
							>
								...
								>
									<svg slot="label" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.28564 14.1921V3.42857H13.7142V14.1921C13.7142 14.6686 13.5208 15.089 13.1339 15.4534C12.747 15.8178 12.3005 16 11.7946 16H4.20529C3.69934 16 3.25291 15.8178 2.866 15.4534C2.4791 15.089 2.28564 14.6686 2.28564 14.1921Z"/><path d="M14.8571 1.14286V2.28571H1.14282V1.14286H4.57139L5.56085 0H10.4391L11.4285 1.14286H14.8571Z"/></svg>
									<span slot="label"><?php _e( 'Delete Query', 'jet-engine' ); ?></span>
								</cx-vui-button>
							</div>
						</div>
					</div>
				...
				<div :class="{ 'jet-engine-query-preview__body': true, 'jet-engine-query-preview__body-updating': updatingPreview }">
					<div class="jet-engine-query-preview__count"><b><?php _e( 'Results Count:', 'jet-engine' ); ?></b> {{ previewCount }}</div>
					<cx-vui-collapse :collapsed="true" class="jet-engine-query-preview__collapse" v-if="previewSqlQueries">
						<template slot="title"><b><?php _e( 'Executed SQL Queries', 'jet-engine' ); ?></b></template>
						<template slot="content">
							<pre>{{ previewSqlQueries }}</pre>
						</template>
					</cx-vui-collapse>
