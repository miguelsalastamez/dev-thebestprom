...
class Tribe__Tickets_Plus__Commerce__WooCommerce__Main extends Tribe__Tickets_Plus__Tickets {
	 ...
	 * Name of the CPT that holds Attendees (tickets holders).
	 *
	 * @deprecated 6.4.0 Use ATTENDEE_OBJECT const instead.
	 *
	 * @var string
	 */
	...
	/**
	 * Meta key used to track the total number of WooCommerce tickets sold for each event.
	 *
	 * @since 6.7.0
	 * @var string
	 */
	 ...
	 * Meta key that relates Attendees and Orders.
	 *
	 * @deprecated 6.4.0 Use ATTENDEE_ORDER_KEY const instead.
	 *
	 * @var string
	 */
	 ...
	 * Meta key that relates Attendees and Events.
	 *
	 * @deprecated 6.4.0 Use ATTENDEE_EVENT_KEY const instead
	 *
	 * @var string
	 */
	...
	public function hooks() {
		...
		add_filter( 'tec_tickets_pre_build_ticket_properties', [ $this, 'filter_pre_build_ticket_properties' ], 10, 2 );
	}
	 ...
	 *
	 * @since 6.8.4 Fixed the issue where the stock was not being synced correctly when a ticket was moved between events.
	 *
	...
	public function create_order_for_moved_ticket( $attendee_id, $src_ticket_type_id, $tgt_ticket_type_id, $src_event_id, $tgt_event_id, $instigator_id ) {
		...
		update_post_meta( $attendee_id, self::ATTENDEE_ORDER_KEY, $new_order->get_id() );
		...
		// Prevent WooCommerce from auto-incrementing total_sales, already incremented by increase_ticket_sales_by().
		$new_order->get_data_store()->set_recorded_sales( $new_order, true );

		// Prevent WooCommerce from auto-reducing stock, already reduced by increase_ticket_sales_by().
		$new_order->get_data_store()->set_stock_reduced( $new_order->get_id(), true );
	...
	public function generate_tickets( $order_id ) {
		 ...
		 *
		 * @since 4.2
		 * @since 6.3.0 Added the $order parameter.
		 *
		 * @param array<string> $generation_statuses The list of order post statuses that should trigger the ticket generation.
		 * @param WC_Order      $order The order object.
		 */
		$generation_statuses = (array) apply_filters( 'event_tickets_woo_ticket_generating_order_stati', $generation_statuses, $order );
		 ...
		 * @since 6.3.0 Added the $order parameter.
		 *
		 * @param array<string> $trigger_statuses order post statuses.
		 * @param WC_Order      $order The order object.
		 */
		$dispatch_statuses = apply_filters( 'event_tickets_woo_complete_order_stati', $dispatch_statuses, $order );
	...
	public function save_ticket( $post_id, $ticket, $raw_data = [] ) {
		...
		if ( empty( $ticket->ID ) ) {
			...
			$product->set_sale_price( $raw_data['ticket_sale_price'] ?? '' );
			...
			$product->set_date_on_sale_from( ! empty( $raw_data['ticket_sale_start_date'] ) ? gmdate( 'Y-m-d 00:00:00', strtotime( wc_clean( $raw_data['ticket_sale_start_date'] ) ) ) : '' );
			$product->set_date_on_sale_to( ! empty( $raw_data['ticket_sale_end_date'] ) ? gmdate( 'Y-m-d 23:59:59', strtotime( wc_clean( $raw_data['ticket_sale_end_date'] ) ) ) : '' );
			$product->save();
		 ...
		 *
		 * @param bool $should_default_ticket_sku Whether to default the ticket SKU.
		 */
		 ...
		 *
		 * @param bool    $can_update_ticket_price Whether the ticket price can be updated.
		 * @param WP_Post $ticket                  The ticket object.
		 */
	 ...
	 * @since 6.7.0 Added a call to decrement the issued tickets count when an attendee is deleted.
	 *
	 * @param int $post_id   The post ID of the event (or post) holding the ticket.
	 * @param int $ticket_id The post ID of the ticket product or the attendee post.
	 *
	 * @return bool
	 */
	public function delete_ticket( $post_id, $ticket_id ) {
		...
		/**
		 * Use this Filter to choose if you want to delete tickets instead of only trashing them.
		 *
		 * @param bool   $trash_ticket Whether to trash the ticket (true) instead of deleting it (false).
		 * @param int    $ticket_id    The post ID of the ticket product.
		 */
		...
		// Increment deleted attendees count if we're deleting an attendee.
		if ( self::ATTENDEE_OBJECT === $ticket_post_type ) {
			Tribe__Tickets__Attendance::instance( $post_id )->increment_deleted_attendees_count();

			// Decrease issued tickets (total sales) count if the product is valid.
			if ( $product_id ) {
				$this->decrement_issued_tickets_count( $product_id );
			}
		}
	...
	/**
	 * Replaces the link to the WooCommerce product with a link to the associated Event
	 * on the order confirmation page, if the product is linked to an Event.
	 *
	 * @since 6.1.5 Added check to ensure only strings are returned.
	 *
	 * @param string  $post_link        The original post link for the WooCommerce product.
	 * @param WP_Post $post             The post object representing the WooCommerce product.
	 * @param bool    $unused_leavename Unused parameter for compatibility.
	 * @param bool    $unused_sample    Unused parameter for compatibility.
	 *
	 * @return string The modified post link pointing to the associated Event,
	 *                or the original post link if no Event is associated or the Event link is invalid.
	 */
	...
	public function front_end_tickets_form( $content ) {
		...
		if ( $expired_tickets >= count( $tickets ) ) {
			 ...
			 * @since 4.7.3
			 *
			 * @param bool    $must_login Whether the user must be logged in.
	 ...
	 */
	public function filter_pre_build_ticket_properties( array $properties, WP_Post $post ): array {
		if ( 'product' !== $post->post_type ) {
			return $properties;
		}
		...
		$event_id = get_post_meta( $post->ID, $this->event_key, true );

		$ticket_object = $this->get_ticket( $event_id, $post->ID );

		$start_date = get_post_meta( $post->ID, '_ticket_start_date', true );
		$end_date   = get_post_meta( $post->ID, '_ticket_end_date', true );
		...
		$props = [
			...
			'price'                 => (float) $product->get_price( 'view' ),
			'regular_price'         => (float) $product->get_regular_price( 'view' ),
			'show_description'      => $ticket_object->show_description(),
			'start_date'            => $start_date,
			'end_date'              => $end_date,
			'sale_price_start_date' => $product->get_date_on_sale_from( 'view' ),
			'sale_price_end_date'   => $product->get_date_on_sale_to( 'view' ),
		...
		return array_merge( $properties, $props );
	}
	 ...
	 *
	 * @param int  $ticket_id The ticket ID.
	 * @param bool $refresh   Whether to refresh the quantity pending.
	 *
	 * @return int
	 */
	 ...
	 *
	 * @param int $existing_stock The existing stock.
	 * @param int $post_id        The post ID.
	 * @param int $ticket_id      The ticket ID.
	 *
	 * @return int
	 ...
	 *
	 * @param object|int $ticket_product The ticket product.
	 *
	 * @return bool|WP_Post
	 */
	...
	public function get_attendee( $attendee, $post_id = 0 ) {
		...
		if (
			...
			|| self::ATTENDEE_OBJECT !== $attendee->post_type
		) {
			return false;
		}
	 ...
	 * @since 6.8.2    Add the optional `$details` parameter to be able to set the checkin time and device_id.
	 *
	 ...
	 */
	public function checkin( $attendee_id, $qr = false, $event_id = null, $details = [] ) {
		$checkin  = null;
		$attendee = $this->get_attendee( $attendee_id );
		$order    = wc_get_order( $attendee['order_id'] );

		$check_in_stati = apply_filters( 'event_tickets_attendees_woo_checkin_stati', [], $this->orm_provider, $attendee['order_id'] );
		if (
			$qr
			&& $order
			&& ! in_array( $order->get_status(), $check_in_stati, true )
		) {
			$checkin = false;
		}
		 ...
		 * @since 5.9.2
		 *
		 * @param bool|null $checkin     Whether the Attendee should be checked in.
		 ...
		 */
		$checkin = apply_filters( 'tec_tickets_attendee_checkin', $checkin, (int) $attendee_id, (int) $event_id, (bool) $qr );
		...
		parent::save_checkin_details( $attendee_id, $qr, $details );
		 ...
		 * @param bool|null $qr          Whether via QR code.
		 */
	...
	public function tribe_attendee_registration_form_class( $classes ) {
		$classes[ self::ATTENDEE_OBJECT ] = 'woo';
	...
	public function woo_attendee_registration_notice_cart_qty_change( $passed_provider, $passed_provider_class, $events ) {
		if (
			...
			|| self::ATTENDEE_OBJECT !== $passed_provider
			|| empty( $events )
	...
	public function add_checkout_links() {
		...
		if ( ! empty( $cart_has_meta ) ) {
			...
			echo sprintf(
				'<a class="tribe-checkout-backlink" href="%1$s">%2$s</a>',
				esc_url( add_query_arg( $provider_slug, self::ATTENDEE_OBJECT, $attendee_registration->get_url() ) ),
	...
	public function attendee_decreases_inventory( array $attendee ) {
		...
		// For refunded and trashed orders inventory is not restocked automatically, we should only decrease if the order was not restocked.
		if ( in_array( $order_status, [ 'refunded', 'trash' ], true ) && tribe_is_truthy( $order->get_meta( $this->restocked_refunded_order ) ) ) {
	 ...
	 *
	 * @since 5.3.1
	 * @since 6.6.0 Add check if the attendee is an array.
	 *
	...
	public function allow_resending_email( $ticket = null, $attendee = null ) {
		if ( empty( $attendee ) || ! is_array( $attendee ) ) {
			// If the attendee is not an array, return false.
	...
	public function filter_cache_listener_save_post_types( array $post_types = [] ): array {
		...
		$post_types[] = self::ATTENDEE_OBJECT;
	 ...
	 * @return array
	 */
	public function get_sale_price_details( int $ticket_id ) {
		$wc_ticket_product = wc_get_product( $ticket_id );

		if ( ! $wc_ticket_product instanceof WC_Product ) {
			return [];
		}

		return [
			'enabled'    => $wc_ticket_product->is_on_sale(),
			'sale_price' => $wc_ticket_product->get_sale_price(),
			'start_date' => $wc_ticket_product->get_date_on_sale_from(),
			'end_date'   => $wc_ticket_product->get_date_on_sale_to(),
		];
	}
	...
	/**
	 * Decrements the count of total WooCommerce tickets issued/sold for an event/post.
	 *
	 * @since 6.7.0
	 *
	 * @param int $post_id The post ID or the WooCommerce product.
	 * @param int $units   Number of units to decrement by (defaults to 1).
	 *
	 * @return void
	 */
	public function decrement_issued_tickets_count( $post_id, $units = 1 ): void {
		if ( ! $post_id ) {
			return;
		}
		...
		$issued = (int) get_post_meta( $post_id, self::TOTAL_SALES, true );
		$result = $issued - (int) $units;
		update_post_meta( $post_id, self::TOTAL_SALES, max( $result, 0 ) );
	}
}
